...
{if $reviewFormResponses}
    {foreach from=$reviewFormElements item=reviewFormElement}
        {if $authorFriendly && !$reviewFormElement->included}
            {continue}
        {/if}
        {assign var="elementId" value=$reviewFormElement->getId()}
        <div>
            <h4 style="font-weight: bold;">{$reviewFormElement->getLocalizedQuestion()|strip_tags|escape}</h4>
        </div>

        {if $reviewFormElement->getLocalizedDescription()}
            <span>{$reviewFormElement->getLocalizedDescription()|strip_tags|escape}</span>
        {/if}
...
{else}
    <div>
        <h4 style='font-weight: bold;'>{translate key="editor.review.reviewerComments"}</h4>
        <em style='font-weight: bold; color:#606060;'>{translate key="submission.comments.forAuthorEditor"}</em>
    </div>
    ...
    {else}
        {foreach from=$submissionComments item=comment}
            <div class='section'><span>{$comment->comments|escape}</span></div>
        {/foreach}
    {/if}
...
<div>
    <h4 style='font-weight: bold;'>{translate key="reviewer.submission.reviewFiles"}</h4>
</div>
